...
const ReviewCard = ({ review }) => {
    const { name, rating, reviewText, picture } = review;
    return (
        <div>
            <div className="md:flex justify-between my-2 md:my-10 card-info shadow-xl">
                <div className="mr-3 md:mr-10">
                    <img className='img-sizing' src={picture} alt="" />
                </div>
                <div className="flex flex-col justify-between">
                    <h3 className='text-2xl font-semibold text-gray-900 mb-2'>{name}</h3>
                    <div className='my-3'>
                        <p className='text-xl text-gray-800'>{reviewText}</p>
                        <br />
                        <div className="flex">
                            <p className='text-xl text-gray-700'>Rating: {rating} </p>
